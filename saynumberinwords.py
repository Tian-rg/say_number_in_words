...
class SayNumberInWords(object):
    ...
    def __checknumber(number):
        ''' This static method is to check if the string from the user 
        is a number. If not, it raises the exception correspondingly.
        ...
        if len(str(number).split(".")) == 2:
            ...
            if len(number2) > supported_number_length[1]:
                raise TooManyDecimalDigits('''Too many decimal digits to
                 handle''')
